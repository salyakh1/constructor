  ...
  useEffect(() => {
    const fetchInvitation = async () => {
      try {
        ...
        const response = await fetch(`/api/invites/${resolvedParams.slug}`)
  ...
  const renderBlock = (block: Block) => {
    ...
    switch (block.type) {
      ...
      case 'photo':
        return (
          <div key={block.id} style={blockStyle}>
            {(block.content as { url?: string; alt?: string }).url ? (
              <img 
                src={(block.content as { url?: string; alt?: string }).url} 
                alt={(block.content as { url?: string; alt?: string }).alt || 'Фото'} 
      ...
      case 'video':
        return (
          <div key={block.id} style={blockStyle}>
            {(block.content as { url?: string; autoplay?: boolean }).url ? (
              <video 
                src={(block.content as { url?: string; autoplay?: boolean }).url} 
                controls 
                autoPlay={(block.content as { url?: string; autoplay?: boolean }).autoplay}
                className="w-full h-full object-cover rounded-lg"
              />
            ) : (
              <div className="w-full h-full bg-gray-200 flex items-center justify-center rounded-lg">
          ...
          <div key={block.id} style={blockStyle}>
            <div className="w-full h-full bg-gradient-to-br from-purple-100 to-pink-100 flex flex-col items-center justify-center rounded-lg border border-purple-200">
              ...
              <div className="text-xs text-purple-800 text-center px-2">
                {(block.content as { title?: string }).title || 'Музыка'}
              </div>
        ...
        return (
          <div key={block.id} style={blockStyle}>
            <div 
              ...
              onClick={() => {
                if ((block.content as { address?: string }).address) {
                  const encodedAddress = encodeURIComponent((block.content as { address?: string }).address!)
            ...
            >
              ...
              <div className="text-xs text-blue-800 text-center px-2 leading-tight">
                {(block.content as { address?: string }).address || 'Укажите адрес'}
              </div>
          ...
          <div key={block.id} style={blockStyle}>
            <div className="w-full h-full bg-gradient-to-br from-orange-100 to-red-100 flex flex-col items-center justify-center rounded-lg border border-orange-200">
              ...
              <div className="text-xs text-orange-800 text-center px-2">
                {(block.content as { targetDate?: string }).targetDate ? (
                  <div>
                    <div className="font-semibold">До события:</div>
                    <div className="text-lg font-bold">
                      {Math.ceil((new Date((block.content as { targetDate?: string }).targetDate!).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))} дней
                    </div>
                  </div>
                ) : (
                  'Таймер'
          ...
          >
            <div className="w-full h-full bg-gradient-to-br from-green-100 to-emerald-100 flex flex-col items-center justify-center rounded-lg border border-green-200">
              ...
              <div className="text-xs text-green-800 text-center px-2">
                <div className="font-semibold mb-1">{(block.content as { question?: string }).question || 'Придете?'}</div>
                {rsvpResponse ? (
                  ...
                  <div className="space-y-1">
                    {(block.content as { options?: string[] }).options?.map((option: string, index: number) => (
                      <motion.button
                        key={index}
            ...
            <div className="w-full h-full bg-gradient-to-br from-yellow-100 to-amber-100 flex flex-col items-center justify-center rounded-lg border border-yellow-200">
              ...
              <div className="text-xs text-amber-800 text-center px-2">
                ...
                <textarea
                  ...
                  placeholder={(block.content as { placeholder?: string }).placeholder || 'Оставьте пожелание'}
