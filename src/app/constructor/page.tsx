...
interface BlockContent {
  ...
  title?: string
  story?: string
  photos?: Array<{ url: string; alt?: string; story?: string }>
  members?: Array<{ name: string; role: string; photo?: string | null; description?: string }>
  question?: string
  options?: string[]
  placeholder?: string
  showDays?: boolean
  ...
  importantDates?: Array<string | { date: string; title: string; description?: string }>
  places?: Array<{ name: string; address: string; coordinates?: { lat: number; lng: number } }>
  [key: string]: unknown
}
...
export default function ConstructorPage() {
  ...
  const [savedInvite, setSavedInvite] = useState<{ id: string; slug: string; qrCodeUrl: string } | null>(null)
                        ...
                        {block.type === 'our-story' && (
                          <div style={{ 
                            ...
                            <div style={{
                              ...
                              {block.content.photos && Array.isArray(block.content.photos) && block.content.photos.length > 0 ? (
                                block.content.photos.map((photo: { url: string; alt?: string; story?: string }, index: number) => (
                                  <div
                                    key={index}
                                    style={{
                                      display: 'flex',
                                  ...
                                  >
                                    ...
                                    <div
                                      style={{
                                        width: Number(block.content.photoSize) || 80,
                                        height: Number(block.content.photoSize) || 80,
                            ...
                            <div style={{
                              ...
                              {block.content.members && block.content.members.length > 0 ? (
                                <div style={{
                                  ...
                                  {(block.content as { members: Array<{ name: string; role: string; photo?: string | null; description?: string }> }).members.map((member, index: number) => (
                                    <div
                                      key={index}
                                      style={{
                                        display: 'flex',
                ...
                <div className='space-y-4 border-t border-gray-200 pt-4'>
                  ...
                  {selectedBlock.type === 'our-story' && (
                    <div className='space-y-4'>
                      ...
                      {/* Размер фотографий */}
                      <div>
                        <label className='block text-sm font-medium text-gray-700 mb-2'>Размер фотографий: {Number(selectedBlock.content.photoSize) || 80}px</label>
                        <input
                          type="range"
                          min="60"
                          max="120"
                          step="10"
                          value={Number(selectedBlock.content.photoSize) || 80}
                          onChange={(e) => {
                            const updated = { 
                              ...selectedBlock, 
                      ...
                      {selectedBlock.content.photos && selectedBlock.content.photos.length > 0 && (
                        <div>
                          ...
                          <div className='space-y-3 max-h-40 overflow-y-auto'>
                            {(selectedBlock.content as { photos: Array<{ url: string; alt?: string; story?: string }> }).photos.map((photo, index: number) => (
                    ...
                    <div className='space-y-4'>
                      ...
                      {selectedBlock.content.members && selectedBlock.content.members.length > 0 && (
                        <div>
                          ...
                          <div className='space-y-3 max-h-40 overflow-y-auto'>
                            {(selectedBlock.content as { members: Array<{ name: string; role: string; photo?: string | null; description?: string }> }).members.map((member, index: number) => (
                              <div key={index} className='p-3 bg-gray-50 rounded-lg border border-gray-200'>
                                <div className='flex justify-between items-start mb-2'>
                                  <div className='text-xs text-gray-600 font-medium'>Участник {index + 1}</div>
                                  <button
                                    onClick={() => {
                                      const updatedMembers = (selectedBlock.content as { members: Array<{ name: string; role: string; photo?: string }> }).members.filter((_, i: number) => i !== index)
                                      const updated = { 
                                        ...selectedBlock, 
